(ns spacex_telemetry.main
  "Kotodama WASM entrypoint for the spacex_telemetry clean-room actor (L4) — Clojure port.
  ...
  facts (`spacex_telemetry.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "spacex_telemetry")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"spacex_telemetry.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "spacex_telemetry-compat" :tier tier :entities entities} 200])
...
(defn start! [] :spacex_telemetry-compat/ready)
